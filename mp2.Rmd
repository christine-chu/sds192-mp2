...
```{r message = FALSE}
positions <- c("DEM","DFL","REP","UNK","OTH")

party_plot_bar <- ggplot(cont_winner2, aes(x = cand_party_affiliation, y = transaction_amt, fill = ge_winner)) +
  geom_bar(position = "dodge", stat = "identity") + #how to change the bar - change the factor levels so that we can change the order in a list
  labs( x = "Party Affiliation", y = "Transaction Amount") + 
  scale_fill_discrete(name = "Election Results", labels = c("Lose", "Win")) +
  scale_x_discrete(limits = positions)+
  scale_y_continuous(limits = c(0, 3000000
), breaks = seq(0, 3000000, 500000), expand = c(0,0), labels = c("0M","0.5M","1M","1.5M","2M","2.5M","3M")) + 
