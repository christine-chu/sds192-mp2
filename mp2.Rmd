...
cont_winner <- contributions1 %>%
  ...
  View() #cannot assign this table to a new dataframe. Are we supposed to write a function for this to replicate it. cont_winner is empty 
...
contributions1 %>%
  group_by(cand_name) %>%
  summarise(n_row = n(), amt = sum(transaction_amt))%>%
   View()
